...
<script>
    ...
    $(document).ready(function() {
        ...
        function permission() {
            if (!`{{ $myAuth->hasPermission('groupproduct', 'store') }}`) {
                $('#add').attr('disabled', 'disabled')
            }
            
            if (!`{{ $myAuth->hasPermission('groupproduct', 'update') }}`) {
                $('#edit').attr('disabled', 'disabled')
            }

            if (!`{{ $myAuth->hasPermission('groupproduct', 'destroy') }}`) {
                $('#delete').attr('disabled', 'disabled')
            }

            if (!`{{ $myAuth->hasPermission('groupproduct', 'export') }}`) {
                $('#export').attr('disabled', 'disabled')
            }

            if (!`{{ $myAuth->hasPermission('groupproduct', 'report') }}`) {
                $('#report').attr('disabled', 'disabled')
            }

            if (!`{{ $myAuth->hasPermission('groupproduct', 'import') }}`) {
        ...
        $('#btnImport').click(function(event) {
            ...
            let url = `${apiUrl}groupproduct/import`
        ...
        $('#formRange').submit(function(event) {
            ...
            getCekExport(params).then((response) => {
                if ($('#rangeModal').data('action') == 'export') {
                    $.ajax({
                        url: `{{ config('app.api_url') }}groupproduct/export?` + params,
                        type: 'GET',
                            ...
                            if (xhr.status === 200) {
                                if (response !== undefined) {
                                    var blob = new Blob([response], {
                                        type: 'groupproduct/vnd.ms-excel'
                                    });
                                    var link = document.createElement('a');
                                    link.href = window.URL.createObjectURL(blob);
                                    link.download = 'laporangroupproduct' + new Date().getTime() + '.xlsx';
                                    link.click();
                                }
                    ...
                    window.open(`{{ route('groupproduct.report') }}?${params}`)
                    submitButton.removeAttr('disabled')
                    $('#processingLoader').addClass('d-none')
                    $('#rangeModal').modal('hide')
                }
        ...
        function getCekExport(params) {
            ...
            return new Promise((resolve, reject) => {
                $.ajax({
                    url: `${apiUrl}groupproduct/export?${params}`,
                    dataType: "JSON",
                    headers: {
                        Authorization: `Bearer ${accessToken}`
                    },
