...
<script>
    ...
    $(document).ready(function() {
        ...
        function permission() {
            ...
            if (!`{{ $myAuth->hasPermission('satuan', 'import') }}`) {
        ...
        $('#formRange').submit(function(event) {
            ...
            getCekExport(params).then((response) => {
                if ($('#rangeModal').data('action') == 'export') {
                    $.ajax({
                        url: `{{ config('app.api_url') }}satuan/export?` + params,
                        type: 'GET',
                            ...
                            if (xhr.status === 200) {
                                if (response !== undefined) {
                                    var blob = new Blob([response], {
                                        type: 'satuan/vnd.ms-excel'
                                    });
                                    var link = document.createElement('a');
                                    link.href = window.URL.createObjectURL(blob);
                                    link.download = 'laporansatuan' + new Date().getTime() + '.xlsx';
                                    link.click();
                                }
                    ...
                    window.open(`{{ route('satuan.report') }}?${params}`)
                    submitButton.removeAttr('disabled')
                    $('#processingLoader').addClass('d-none')
                    $('#rangeModal').modal('hide')
                }
        ...
        function getCekExport(params) {
            ...
            return new Promise((resolve, reject) => {
                $.ajax({
                    url: `${apiUrl}satuan/export?${params}`,
                    dataType: "JSON",
                    headers: {
                        Authorization: `Bearer ${accessToken}`
                    },
