...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#btnSubmit').click(function(event) {
            ...
            data.push({
                name: `armadaid`,
                value: form.find(`[name="armadaid"]`).val()
            })

            data.push({
                name: `armadanama`,
                value: form.find(`[name="armadanama"]`).val()
            })
            ...
            switch (action) {
                case 'add':
                    method = 'POST'
                    url = `${apiUrl}transaksiarmada`
                    break;
                case 'edit':
                    method = 'PATCH'
                    url = `${apiUrl}transaksiarmada/${Id}`
                    break;
                case 'delete':
                    method = 'DELETE'
                    url = `${apiUrl}transaksiarmada/${Id}`
    ...
    function createTransaksiArmada() {
        let form = $('#crudForm')
        $('#crudModal').find('#crudForm').trigger('reset')
        $('#crudModalTitle').text('create transaksi armada')
    ...
    function editTransaksiArmada(id) {
        let form = $('#crudForm')
        $('.modal-loader').removeClass('d-none')
        form.data('action', 'edit')
        form.trigger('reset')
        $('#crudModalTitle').text('edit transaksi armada')
    ...
    function deleteTransaksiArmada(id) {
        let form = $('#crudForm')

        $('.modal-loader').removeClass('d-none')
        $('#crudModalTitle').text('delete transaksi armada')
    ...
    function viewTransaksiArmada(userId) {
        let form = $('#crudForm')
        $('.modal-loader').removeClass('d-none')
        ...
        form.find('#btnSubmit').html(`
            ...
            `)
        form.find(`.sometimes`).hide()
        $('#crudModalTitle').text('View Transaksi Armada')
        $('.is-invalid').removeClass('is-invalid')
        $('.invalid-feedback').remove()

        Promise
            .all([
                ShowTransaksiArmada(form, userId)
            ])
            .then(() => {
                $('#crudModal').modal('show')
    ...
    function getMaxLength(form) {
        ...
        return new Promise((resolve, reject) => {
            $.ajax({
            url: `${apiUrl}transaksiarmada/field_length`,
            method: 'GET',
            dataType: 'JSON',
            headers: {
                'Authorization': `Bearer ${accessToken}`
    ...
    function showTransaksiArmada(form, userId) {
        return new Promise((resolve, reject) => {
            $('#detailList tbody').html('')
            $.ajax({
                url: `${apiUrl}transaksiarmada/${userId}`,
                method: 'GET',
                dataType: 'JSON',
                headers: {
                    Authorization: `Bearer ${accessToken}`
