        ...
        <form action="" method="post">
          <div class="modal-body modal-master modal-overflow" style="overflow-y: auto; overflow-x: auto;">
            ...
            <div class="row form-group">
              <div class="col-12 col-md-2">
                <label class="col-form-label">
                  no bukti pesanan final <span class="text-danger">*</span>
                </label>
              </div>
              <div class="col-12 col-md-10">
                <input type="text" name="nobuktipesananfinal" class="form-control lg-form filled-row " readonly>
              </div>
            </div>
            <div class="row form-group">
...
<script>
  ...
  function setTotalHarga(element, id=0) {
    ...
    let hargasatuan = AutoNumeric.getNumber(element.parents('tr').find(`td [name="harga[]"]`)[0])
  ...
  function setHarga(element){
    ...
    // setharga = totalharga / qty
    
    console.log("total harga",totalharga)
    console.log("total qty",qty)
    // initAutoNumericNoDoubleZero($(`#harga`).val(setharga))
      ...
      $.ajax({
        ...
        success: response => {
          ...
          if (detectDeviceType() == "desktop") {
            ...
            <th class="wider-qty text-right"  style="width: 300px; min-width: 100px;">Qty Retur</th>
        ...
        detailRow.on('input', 'input[name="qtyretur[]"]', function() {
            ...
            $.each(response.detail, (index, detail) => {
              ...
              <td  class="table-bold">
                <label class="col-form-label mt-2 label-mobile">${index+1}. &ensp; produk</label>
                  <input type="hidden" name="productid[]" class="form-control filled-row detail_stok_${selectIndex}">
    ...
    $(`.item-lookup${rowLookup}`).lookup({
      ...
      onSelectRow: (item, element) => {
        ...
        let currentRow = $(element).closest('tr');
          ...
          let qtyCek = (currentRow.find('input[name="qtyretur[]"]').val() == '') ? 0 : parseFloat(currentRow.find('input[name="qtyretur[]"]').val().replace(/,/g, ''));
    ...
    $(`.satuan-lookup${rowLookup}`).lookup({
      ...
      onSelectRow: (satuan, element) => {
        ...
        element.val(satuan.nama);
          ...
          let qtyReturCek = (currentRow.find('input[name="qtyreturjual[]"]').val() == '') ? 0 : parseFloat(currentRow.find('input[name="qtyreturjual[]"]').val().replace(/,/g, ''));
