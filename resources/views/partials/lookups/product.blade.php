<?php
if (isset($id)) { ?>
<table id="productLookup<?= $id ?>" class="lookup-grid"></table>
<?php
} else { ?>
<table id="productLookup" class="lookup-grid"></table>
<?php } ?>
...
<script>
    ...
    selector = $(`#productLookup{{ isset($id) ? $id : null }} `)
    ...
    selector.jqGrid({
        url: `{{ config('app.api_url') . 'product' }}`,
        mtype: "GET",
        styleUI: 'Bootstrap4',
        iconSet: 'fontAwesome',
        datatype: "json",
