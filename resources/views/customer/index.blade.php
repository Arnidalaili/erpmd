    ...
    $(document).ready(function() {
        $("#jqGrid").jqGrid({
                ...
                colModel: [{
                    ...
                    {
                        label: 'HARGA PRODUCT',
                        name: 'hargaproductmemo',
                        width: 100,
                        stype: 'select',
                        searchoptions: {
                            value: `<?php
                                    $i = 1;
                                    foreach ($data['hargaproductmemo'] as $hargaproduct) :
                                        echo "$hargaproduct[param]:$hargaproduct[parameter]";
                                        if ($i !== count($data['hargaproductmemo'])) {
                                            echo ';';
                                        }
                                        $i++;
                                    endforeach;
            ...
            `,
                        ...
                        formatter: (value, options, rowData) => {
                            let hargaproduct = JSON.parse(value)
                            ...
                            let formattedValue = $(`
                <div class="badge" style="background-color: ${hargaproduct.WARNA}; color: #fff;">
                  <span>${hargaproduct.SINGKATAN}</span>
                </div>
              `)
                        ...
                        cellattr: (rowId, value, rowObject) => {
                            ...
                            let hargaproduct = JSON.parse(rowObject.hargaproductmemo)

                            if (!hargaproduct) {
                                return ''
                            }

                            return ` title="${hargaproduct.MEMO}"`
                        }
                    },
                    {
                        label: 'GROUP',
                    ...
                    {
                        label: 'DISCOUNT PERSEN',
                        name: 'discountpersen',
                        align: 'left',
                    },
                    {
                        label: 'DISCOUNT NILAI',
                        name: 'discountnilai',
                        align: 'right',
                    },
                    {
        ...
        function permission() {
            ...
            if (!`{{ $myAuth->hasPermission('customer', 'import') }}`) {
        ...
        $('#btnImport').click(function(event) {
            ...
            let url = `${apiUrl}customer/import`
        ...
        $('#formRange').submit(function(event) {
            ...
            getCekExport(params).then((response) => {
                if ($('#rangeModal').data('action') == 'export') {
                    $.ajax({
                        url: `{{ config('app.api_url') }}customer/export?` + params,
                        type: 'GET',
