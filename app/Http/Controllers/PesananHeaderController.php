...
class PesananHeaderController extends MyController
{
    ...
    public function index(Request $request)
    {
        ...
        $data = [
            'pagename' => 'Menu Utama Pesanan Header',
            'statusmemo' => $this->combo('list', 'STATUS', 'STATUS'),
            'status2memo' => $this->combo('list', 'STATUS2', 'STATUS2'),
        ];

        return view('pesananheader.index', compact('title', 'data'));
    }
    ...
    public function report(Request $request)
    {
        ...
        $header = Http::withHeaders($request->header())
            ->withOptions(['verify' => false])
            ->withToken(session('access_token'))
            ->get(config('app.api_url') . 'pesananheader/' . $id . '/report')['data'];
        ...
        $details = Http::withHeaders($request->header())
            ->withOptions(['verify' => false])
            ->withToken(session('access_token'))
            ->get(config('app.api_url') . 'pesanandetail', $detailParams)['data'];
